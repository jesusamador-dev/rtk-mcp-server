...
*   **Un solo universo de archivos:** `rtk_grep`, `rtk_find` y el índice recorren exactamente lo mismo (`ignore` + `.gitignore`, sin `.git`, `.rtk-index` ni `openspec/changes/archive/`). Que dos herramientas dieran alcances distintos sobre la misma ruta obligaba a saberlo de antemano para no leer mal el resultado.
*   **Respuestas acotadas:** ninguna herramienta puede devolverte 29 KB por sorpresa. `rtk_find` topa en 400 archivos, `rtk_grep` degrada a conteo por archivo cuando listar las líneas costaría más de ~4 KB, y `symbol_lookup`/`codebase_search` tienen presupuesto de salida (pasado el cual muestran firma o ancla en vez del cuerpo). Todas dicen explícitamente qué dejaron fuera y cómo pedirlo.
...
| `rtk_grep` | Busca un patrón (regex) con **el mismo alcance que `rtk_find` y el índice** (respeta `.gitignore`, excluye `.git`, `.rtk-index` y el histórico de OpenSpec). Devuelve las líneas, o el **conteo por archivo** si son muchas o si pides `count: true`. |
...
| `rtk_grep` | Las mismas líneas en formato `path:línea:texto` (lo que habría impreso `grep -rn`). En modo conteo **no se reporta ahorro**: la información no es equivalente |
...
**El ahorro solo se reporta cuando la información es equivalente.** Si `rtk_grep` devuelve conteos en lugar de líneas, no compara contra `grep -rn`: no entregó lo mismo, así que un porcentaje ahí sería tramposo. Si un baseline no se puede medir (o la llamada falla), se usa el coste real: 0 % de ahorro. **Los ahorros negativos se reportan como tales**: si la respuesta salió más cara que su rival, lo verás con signo menos.
